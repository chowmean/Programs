...
int MUL_strassen(int **P, int **Q, int **R, int n);
...
// Function to multiply matrix using Strassen's Algorithm
int MUL_strassen(int **p, int **q, int **r, int n)
{
    int n1,n2,n3;
    if(n<= 2)
...
// now we will taking submatrices A ,B,C,D,E,F,G,H of n/2 size		
       for (int i = 0; i < n / 2; i++)
       {
         for (int j = 0; j < n / 2; j++)
          {
                    a[i][j] = p[i][j];               
                    b[i][j] = p[i][j + n / 2];        //  | A            B   |   | E            F   |    A,B,C,D,E,F,G,H  are sub matrix of n/2 X n/2
                    c[i][j] = p[i + n/ 2][j];         //  |                  |   |                  |
                    d[i][j] = p[i + n / 2][j + n/ 2]; //  | c            D   |   | G            H   |
                                                      //  |                  |
                    e[i][j] = q[i][j];                
                    f[i][j] = q[i][j + n / 2];
                    g[i][j] = q[i + n/ 2][j];
...
//...using Strassen algo.....here we will calculate M1,M2,M3,M4,M5,M6,M7 with the help of this we will calcualte I,J,K,L 
...
///   After calculating I,J,K,L we put it into the resulttant marix		
       for (int i = 0; i < n/2 ; i++)
        {
         for (int j = 0 ; j < n/2 ; j++)
          {
...
// now deleting each dynamic matrices
       for ( int i = 0; i < n2; i++)
       {
         delete [] a[i];
		 delete [] b[i];
